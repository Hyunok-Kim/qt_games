...
GameScene::GameScene(QObject *parent)
    : QGraphicsScene{parent}, game(), timePerFrame(1000.0f/60.0f), m_isMuted(false)
{
...
void GameScene::drawScore()
{
    ...
    QGraphicsPixmapItem *hundredthPartScoreItem = new QGraphicsPixmapItem(game.m_numbersPixmap.copy(hendredthPartValue*32, 0, 32, 32));
    hundredthPartScoreItem->moveBy(Game::RESOLUTION.width()-3*32, scorePixmapItem->y()+32);
...
void GameScene::keyPressEvent(QKeyEvent *event)
{
    if ( !event->isAutoRepeat() ) {
        ...
        case Qt::Key_Y:
            //renderScene(); //uncomment to make screenshot
            break;
        default:
            break;
        }
